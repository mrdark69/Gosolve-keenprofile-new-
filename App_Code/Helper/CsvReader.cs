...
public class GsCsvReader
{
    ...
    public GsCsvReader(string strPath, string strFilename)
    {
        this.Path = strPath;
        ...
        using (var reader = new CsvFileReader(HttpContext.Current.Server.MapPath(this.Path + this.FileName)))
        {
            ...
            while (reader.ReadRow(columns))
            {
                if (i > 0)
                {
                    ...
                    dt.Rows.Add(obj);
                    //dr = dt.NewRow();
                    //dr["FirstName"] = columns[0].ToString();
                    //dr["Email"] = columns[1].ToString();
                    //dt.Rows.Add(dr);
                ...
                //dr["FirstName"] = columns[0].ToString();
                //dr["Email"] = columns[1].ToString();
                // dt.Columns[i].ColumnName = columns[""]
                i = i + 1;
                // TODO: Do something with columns' values
            }
    ...
    private  DataTable CsvCreateDataTable()
    {
        ...
        //HDR = No mean = get first colume as well;
        strConnString = "Provider=Microsoft.ACE.OLEDB.12.0; Data Source=" + HttpContext.Current.Server.MapPath(this.Path) +
